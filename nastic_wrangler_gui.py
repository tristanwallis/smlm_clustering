...
layout = [
[sg.T("NASTIC Wrangler",font=("Any",24))],
[sg.FolderBrowse("Browse",key="-B1-",target="-H1-",initial_folder=initialdir),sg.T("Choose directory 1",key = "-T1-",size=(20,1)),sg.T("Short:"),sg.Input("",key="-I1-",size=(10,1)),sg.ColorChooserButton("Col1",key="-COLOR1CHOOSE-",button_color=("gray",col1),target = "-COL1-"),sg.T("Exclude"),sg.Combo([""] ,key="-C1-"),sg.Input("",key="-H1-",visible=False,enable_events=True),sg.Input(col1,key ="-COL1-",enable_events=True,visible=False)],
[sg.FolderBrowse("Browse",key="-B2-",target="-H2-",initial_folder=initialdir),sg.T("Choose directory 2",key = "-T2-",size=(20,1)),sg.T("Short:"),sg.Input("",key="-I2-",size=(10,1)),sg.ColorChooserButton("Col2",key="-COLOR2CHOOSE-",button_color=("gray",col2),target = "-COL2-"),sg.T("Exclude"),sg.Combo([""],key="-C2-"),sg.Input("",key="-H2-",visible=False,enable_events=True),sg.Input(col2,key ="-COL2-",enable_events=True,visible=False)],
...
window = sg.Window("NASTIC Wrangler {}".format(last_changed),layout)
popup.close()
...
while True:
	...
	# Update buttons
	window.Element("-COLOR1CHOOSE-").update(button_color=("white",col1))
	window.Element("-COLOR2CHOOSE-").update(button_color=("white",col2))
	...
	if event == "-B4-":
		...
		with open("nastic_wrangler_output_{}.tsv".format(stamp),"w",encoding='utf8') as outfile:
			outfile.write("NASTIC / SEGNASTIC WRANGLER - Tristan Wallis t.wallis@uq.edu.au\n")
	...
	# Help
	if event in ('-B6-'): 
		sg.Popup("NASTIC WRANGLER HELP",
		"This program allows visualising of the metrics.tsv files produced by NASTIC and SEGNASTIC. Comparison bar plots for each metric and statistical significance (t-test) are shown.",
		"Aggregate data: aggregates individual cluster metrics across all samples. N = total number of clusters",
		"Average data: aggregates the averaged cluster metrics for each sample. N = number of samples"	,
		...
		"Give each experimental condition a colour and a short name which will appear in subsequent plots",
